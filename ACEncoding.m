function [seq,seq_len]=ACEncoding(x,y) 
Z=x;
v0=y;
R=abs(y);
if R==0 ;amplen=0;
...
% elseif (Z==0 && amplen==0); codelen=4;accode=10; % EOB
...
% elseif  (Z==15 && amplen==0);codelen=11;accode=2041; % ZRL
...
if v0>0
    seq=[dec2bin(accode,codelen),dec2bin(R,amplen)];
elseif v0<0
    temp = dec2bin(R,amplen);
    for i=1:length(temp)
        if (temp(i)=='0') ;temp(i)='1';
        else ;temp(i)='0';
        end
    end
    seq=[dec2bin(accode,codelen),temp];
else
    seq=[dec2bin(accode,codelen)]; % 2024b
end
seq_len = length(seq);
return;
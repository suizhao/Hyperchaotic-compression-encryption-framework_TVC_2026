function [seq,seq_len]=DCEncoding(value) 
val=abs(value);
if (val==0);amplen=0;codelen=2;dccode=0;
...
if value>0
    seq=[dec2bin(dccode,codelen),dec2bin(val,amplen)];
elseif value<0
    temp = dec2bin(val,amplen);
    for i=1:length(temp)
        if (temp(i)=='0') ;temp(i)='1';
        else ;temp(i)='0';
        end
    end
    seq=[dec2bin(dccode,codelen),temp];
else
    seq=[dec2bin(dccode,codelen)]; % 2024b版本dec2bin(0,0)='0'
end
seq_len=length(seq);
return;